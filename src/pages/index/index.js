...
class IatRecorder {
  ...
  recorderStop() {
    ...
    getCommand({"commond":msg}).then(commandData => {
      ...
      imWebSocket.send(JSON.stringify(commandData));
    });
...
// 连接websocket-IM的websocket
function connectIMWebSocket() {
   let url = "ws://localhost:8888/websocket"
  
    let imWebSocket
    if ('WebSocket' in window) {
      imWebSocket = new WebSocket(url)
    } else if ('MozWebSocket' in window) {
      imWebSocket = new MozWebSocket(url)
    } else {
      alert('浏览器不支持WebSocket')
      return null
    }
    ...
    //this.setStatus('init')
    imWebSocket.onopen = e => {
      console.log("Im Websocket Connection open...");
      //this.setStatus('ing')
    }
    imWebSocket.onmessage = e => {
      console.log("Im Websocket onmessage");
    }
    imWebSocket.onerror = e => {
      console.log("Im Websocket Connection error:"+ e);
    }
    imWebSocket.onclose = e => {
      console.log("Im Websocket Connection closed... ");
    }
    return imWebSocket
}
...
let imWebSocket = connectIMWebSocket()
let countInterval
